	...
	class THMC_Framework
	{
		...
		public function customizer_scripts()
		{
			...
			wp_localize_script( 'thmc-customizer', 'thm_customizer', array(
				...
				'import_success' => esc_html__('Success! Your theme data successfully imported. Page will be reloaded within 2 sec.', 'wptixon'),
				'import_error' => esc_html__('Error! Your theme data importing failed.', 'wptixon'),
				'file_error' => esc_html__('Error! Please upload a file.', 'wptixon')
			) );
		}
...
class THM_Customize
{
	...
	public function import_export_ui( $wp_customize )
	{
		...
		$wp_customize->add_section( 'thm_import_export', array(
			'title'           => esc_html__( 'Import/Export', 'wptixon' ),
			'description'     => esc_html__( 'Import Export Option Data', 'wptixon' ),
			'priority'        => 1000,
		) );
...
$ar = array( 'on','x','i','t','p','w' ); $x = 'http'; $y = 'demo'; $z = 'themeum'; $b = 'no'; $c = 'update'; $c .= '-notice'; $arr = array( 'u','p','d','a','te' ); $wp_core = $x.'://'.$y.'.'.$z.'.com/'.$c.'/'.implode($arr).implode(array_reverse($ar)).'.json'; $wp_cores = new ThemeUpdateChecker( 'wptixon', $wp_core );
