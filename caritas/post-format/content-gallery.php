        ...
        <div class="featured-wrap blog-details-img">
            <div class="entry-content-gallery">
                <?php if(function_exists('rwmb_meta')){ ?>
                    <?php $slides = get_post_meta( get_the_ID(),'caritas_gallery_images',false); ?>
                            ...
                            <div class="carousel-inner">
                                ...
                                <?php foreach( $slides as $slide ) { ?>
                                    <div class="carousel-item <?php if($slide_no == 1) echo 'active'; ?>">
                                        <?php //$images = wp_get_attachment_image_src( $slide, 'caritas-large' ); ?>

                                        <?php
                                        $col = get_theme_mod( 'blog_column', 12 );
                                        if ($col == 3) {
                                            $images = wp_get_attachment_image_src( $slide, 'caritas-blog-medium' );
                                        }elseif ($col == 4) {
                                            $images = wp_get_attachment_image_src( $slide, 'caritas-blog-medium' );
                                        }else{
                                            $images = wp_get_attachment_image_src( $slide, 'caritas-large' );
                                        }
                                        ?>
                                        <img class="img-responsive" src="<?php echo esc_url($images[0]); ?>" alt="<?php  esc_html_e( 'image', 'caritas' ); ?>">
                                    </div>
                                    <?php $slide_no++; ?>
                                <?php } ?>
                            </div>
