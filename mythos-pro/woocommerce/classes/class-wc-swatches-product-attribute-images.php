	...
	*/
	...
	public function init_attribute_image_selector() {
		...
		if (strpos($screen->id, 'pa_') !== false) :
			...
			$this->taxonomy = sanitize_text_field(wp_unslash( $_REQUEST['taxonomy'] ));
	...
	public function woocommerce_add_attribute_thumbnail_field() {
		...
		<div class="form-field ">
			<label for="product_attribute_swatchtype_<?php echo esc_attr($this->meta_key); ?>">Swatch Type</label>
			<select name="product_attribute_meta[<?php echo esc_attr($this->meta_key); ?>][type]" id="product_attribute_swatchtype_<?php echo esc_attr($this->meta_key); ?>" class="postform">
			...
			<script type="text/javascript">
				jQuery(document).ready(function($) {
					...
					$('#product_attribute_swatchtype_<?php echo esc_attr($this->meta_key); ?>').change(function() {
		...
		<div class="form-field swatch-field swatch-field-phoen_color" style="overflow:visible;display:none;">
			<div id="swatch-photo" class="<?php echo esc_attr(sanitize_title($this->meta_key)); ?>-phoen_color">
                <label><?php esc_html_e('Color:', 'mythos-pro'); ?></label>   
                <div >
					<input type="hidden" id="product_attribute_<?php echo esc_attr($this->meta_key); ?>" name="product_attribute_meta[<?php echo esc_attr($this->meta_key); ?>][phoen_color]" />
					<input type='text' class="text_for_swatches" name="product_attribute_meta[<?php echo esc_attr($this->meta_key); ?>][phoen_color]" value="" >
                </div>
				<script type="text/javascript">
					jQuery(document).ready(function(){
	...
	public function woocommerce_edit_attributre_thumbnail_field($term, $taxonomy) {
		...
		<tr class="form-field ">
			<th scope="row" valign="top"><label><?php esc_html_e('Type', 'mythos-pro'); ?></label></th>
			<td>
				<label for="product_attribute_swatchtype_<?php echo esc_attr($this->meta_key); ?>">Swatch Type</label>
				<select name="product_attribute_meta[<?php echo esc_attr($this->meta_key); ?>][type]" id="product_attribute_swatchtype_<?php echo esc_attr($this->meta_key); ?>" class="postform">
					<option <?php selected('none', $swatch_term->get_type()); ?> value="-1"><?php esc_html_e('None', 'mythos-pro'); ?></option>
					<option <?php selected('phoen_color', $swatch_term->get_type()); ?> value="phoen_color"><?php esc_html_e('Color', 'mythos-pro'); ?></option>
				</select>
		...
		<tr class="form-field swatch-field swatch-field-phoen_color" style="overflow:visible;<?php echo esc_html($style); ?>">
			<th scope="row" valign="top"><label><?php esc_html_e('Color', 'mythos-pro'); ?></label></th>
			<td>
				<div >
					<input type="hidden" id="product_attribute_<?php echo esc_attr($this->meta_key); ?>" name="product_attribute_meta[<?php echo esc_attr($this->meta_key); ?>][phoen_color]" value="<?php echo wp_kses($swatch_term->phoen_color); ?>" />
					<input type='text' class="text_for_swatches" name='product_attribute_meta[<?php echo esc_attr($this->meta_key); ?>][phoen_color]' value="<?php echo wp_kses($swatch_term->phoen_color); ?>" >
				</div>
				<script type="text/javascript">
				jQuery(document).ready(function(){
	...
	public function woocommerce_attribute_thumbnail_field_save($term_id, $tt_id, $taxonomy) {
		if (isset($_POST['product_attribute_meta'])) {
			...
			$metas = sanitize_text_field(wp_unslash($_POST['product_attribute_meta']));
	...
	public function woocommerce_product_attribute_columns($columns) {
		...
		$new_columns[$this->meta_key] = __('Thumbnail', 'mythos-pro');
		unset($columns['cb']);
