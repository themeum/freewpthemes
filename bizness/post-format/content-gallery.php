                        ...
                        <div id="blog-gallery-slider<?php echo get_the_ID(); ?>" class="carousel slide blog-gallery-slider">
                            ...
                            <div class="carousel-inner">
                                ...
                                <div class="carousel-item <?php if($slide_no == 1) echo 'active'; ?>">
                                    <?php //$images = wp_get_attachment_image_src( $slide, 'bizness-large' ); ?>

                                    <?php 
                                        $col = get_theme_mod( 'blog_column', 12 );
                                        if ($col == 3) {     
                                            $images = wp_get_attachment_image_src( $slide, 'bizness-blog-medium' );      
                                        }elseif ($col == 4) {
                                            $images = wp_get_attachment_image_src( $slide, 'bizness-blog-medium' );   
                                        }else{  
                                            $images = wp_get_attachment_image_src( $slide, 'bizness-large' );  
                                        }
                                    ?> 
                                    <img class="img-responsive" src="<?php echo esc_url($images[0]); ?>" alt="<?php  esc_html_e( 'image', 'bizness' ); ?>">
                                </div>
                                <?php $slide_no++; ?>
                                <?php } ?>
                            </div>
