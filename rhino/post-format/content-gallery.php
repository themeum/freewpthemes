...
<?php endif; ?>
    ...
    <div class="featured-wrap blog-details-img">
        <div class="entry-content-gallery">
            <?php if(function_exists('rwmb_meta')){ ?>
                <?php $slides = get_post_meta( get_the_ID(),'rhino_gallery_images',false); ?>
                        ...
                        <div id="blog-gallery-slider<?php echo get_the_ID(); ?>" class="carousel slide blog-gallery-slider">
                            ...
                            <div class="carousel-inner">
                                ...
                                <div class="carousel-item <?php if($slide_no == 1) echo 'active'; ?>">
                                    <?php //$images = wp_get_attachment_image_src( $slide, 'rhino-large' ); ?>

                                    <?php 
                                        $col = get_theme_mod( 'blog_column', 12 );
                                        if ($col == 3) {     
                                            $images = wp_get_attachment_image_src( $slide, 'rhino-blog-medium' );      
                                        }elseif ($col == 4) {
                                            $images = wp_get_attachment_image_src( $slide, 'rhino-blog-medium' );   
                                        }else{  
                                            $images = wp_get_attachment_image_src( $slide, 'rhino-large' );  
                                        }
                                    ?> 
                                    <img class="img-responsive" src="<?php echo esc_url($images[0]); ?>" alt="<?php  esc_html_e( 'image', 'rhino' ); ?>">
                                </div>
                                <?php $slide_no++; ?>
                                <?php } ?>
                            </div>
    ...
    <div class="rhino-blog-title clearfix">
        <?php
            if (is_single()) { ?>
