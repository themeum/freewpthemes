...
class Themeum_About_Widget extends WP_Widget{
	...
	public function __construct() {
		parent::__construct( 'Themeum_About_Widget', esc_html__("Themeum About Us Widgets",'aresmurphy'), array('description' => esc_html__("This About Us Widgets",'aresmurphy')) );
	}
	...
	public function widget( $args, $instance ) {
		...
		if($instance['about_img1']) {
			echo '<img src="'. esc_url(get_site_url()) . $instance['about_img1'].'" class="img-responsive" alt="'.esc_html__('image','aresmurphy').'">';
		}
		...
		?>	
			<ul class="themeum-about-info">
				<?php if( isset($instance['about_email']) && $instance['about_email'] ) { ?>
					<li><span><?php esc_html_e('Email: ', 'aresmurphy'); ?></span><?php echo $instance['about_email']; ?></li>
				<?php } ?>				

				<?php if( isset($instance['fax_no']) && $instance['fax_no'] ) { ?>
					<li><span><?php esc_html_e('Fax: ', 'aresmurphy'); ?></span><?php echo $instance['fax_no']; ?></li>
				<?php } ?>				

				<?php if( isset($instance['phone_no']) && $instance['phone_no'] ) { ?>
					<li><span><?php esc_html_e('Phone: ', 'aresmurphy'); ?></span><?php echo $instance['phone_no']; ?></li>
				<?php } ?>				
	...
	public function form( $instance )
	{
	   ...
	   ?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php _e('Title :', 'aresmurphy'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php echo esc_attr($instance['title']); ?>" style="width:100%;" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'about_img1' )); ?>"><?php esc_html_e( 'About Image URL', 'aresmurphy' ); ?></label>
			...
			<input type="hidden" id="<?php echo $this->get_field_id('about_img1');?>" name="<?php echo $this->get_field_name('about_img1');?>" class="<?php echo $this->get_field_id('about_img1');?>" value="<?php echo $instance['about_img1']; ?>"/>
 			<button id="<?php echo $this->get_field_id('about_img1');?>" class="custom-upload button" data-url="<?php echo esc_url(get_site_url()); ?>"><?php echo esc_html__('Upload image','aresmurphy'); ?></button>
 			<img class="<?php echo $this->get_field_id('about_img1');?>" src="<?php echo esc_url(get_site_url()) . $instance['about_img1']; ?> "/>
		</p>
		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'about_text' )); ?>"><?php esc_html_e('About Text :', 'aresmurphy'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'about_email' )); ?>"><?php esc_html_e('Email ID: ', 'aresmurphy'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'fax_no' )); ?>"><?php esc_html_e('Fax No: ', 'aresmurphy'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'phone_no' )); ?>"><?php esc_html_e('Phone No: ', 'aresmurphy'); ?></label>
