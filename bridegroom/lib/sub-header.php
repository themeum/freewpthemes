<?php
    ...
    $subtext = '';
    global $post;
    if(!function_exists('bridegroom_call_sub_header')){ 
        function bridegroom_call_sub_header(){
                ...
                if(!empty($image_attached)){
                    ...
                    if(empty($sub_img[0])){
                        ...
                        if( get_post_meta(get_the_ID(),"themeum_subtitle_color",true ) == ''){
                            $output = bridegroom_call_sub_header();
                        }
                    }
                }else{
...
$subheader = get_theme_mod( 'subheader_style', true ); ?>
    ...
    <div class="subtitle-cover sub-title <?php echo $subheader; ?>" <?php print $output;?>>
        <div class="container">       
            <div class="row">
                        ...
                        <?php
                            ...
                            if(isset($wp_query->queried_object->name)){
                                if (get_theme_mod( 'header_title_enable', true )) {
                                    if($wp_query->queried_object->name != ''){
                                        if($wp_query->queried_object->name == 'product' ){
                                            echo '<h2 class="page-leading">'.esc_html__('Shop','bridegroom').'</h2>';
                                        }else{
                                            ...
                                            if($category) {
                                                ...
                                                if ($option_name) {
                                                    echo '<span class="bridegroom-icons"><i class="back-'.get_option($option_name).'"></i></span>';
                                                }
                                ...
                                if( is_search() ){
                                    ...
                                    if (get_theme_mod( 'header_title_enable', true )) {
                                        $first_char = esc_html__('Search','bridegroom');
                                    ...
                                    if (get_theme_mod( 'header_title_enable', true )) {
                                        if (get_post_type() == 'event') {
                                            ...
                                            echo '<h2 class="page-leading">'. esc_html__( 'Gallery','bridegroom' ).'</h2>';
                                        }
                                        elseif(get_post_type() == 'product'){
                                            ...
                                            if ($terms) {
                                                foreach ( $terms as $term )
                                                    ...
                                                    echo '<h2 class="page-leading">'. esc_html__( 'Product Details','bridegroom' ).'</h2>';;
                                                    if ($product_subtitle) {
                                                        echo '<p>'.get_option($product_subtitle).'</p>';
                                                    }
                                            }
                                            ...
                                            if (get_theme_mod( 'header_title_text', 'Latest Blog' )){
                                                echo '<h2 class="page-leading">'.__('Single Blog Post', 'bridegroom').'</h2>';
                                            }
                                        }
                                    }
                                    ...
                                    if (get_theme_mod( 'header_title_enable', true )) {
                                        $page_intro = get_post_meta($post->ID, 'themeum_page_intro', true);
                                        echo '<span class="intro-text">'. $page_intro .'</span>';
                                        echo '<h2 class="page-leading">'.get_the_title().'</h2>';
                                    }
                                }
                            }
