<?php
    ...
    if(!function_exists('floox_call_sub_header')){
        function floox_call_sub_header(){
            ...
            <div class="col-12 text-center">
                    ...
                    if(isset($wp_query->queried_object->name)){
                        if (get_theme_mod( 'header_title_enable', true )) {
                            if($wp_query->queried_object->name != ''){
                                if($wp_query->queried_object->name == 'product' ){
                                    echo '<h2>'.esc_html__('Shop','floox').'</h2>';
                                }else{
                                    echo '<h3 class="page-subleading">'. esc_html__('Category: ' ).$wp_query->queried_object->name.'</h3>';
                                }
                            }else{
                        ...
                        if( is_search() ){
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                $text = '';
                                $first_char = esc_html__('Search','floox');
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                if (get_theme_mod( 'header_title_text', 'Blog' )){
                                    echo '<h3 class="page-subleading">'. get_theme_mod( 'header_title_text','Blog' ).'</h3>';
                                }
                            }
                        }
                        // else if( is_page() ){
                        ...
                        // }
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                if (get_post_type() == 'event') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Event Details','floox' ).'</h2>';
                                } elseif (get_post_type() == 'album') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Albums','floox' ).'</h2>';
                                } elseif (get_post_type() == 'gallery') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Gallery','floox' ).'</h2>';
                                } elseif (get_post_type() == 'performer') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Performer','floox' ).'</h2>';
                                }elseif(get_post_type() == 'product'){
                                    echo '<h2>'.esc_html__('Product Details','floox').'</h2>';
                                }elseif(get_post_type() == 'portfolio'){
                                    echo '<h2>'.esc_html__('','floox').'</h2>';
                                    if ( $subtext != "" ){
