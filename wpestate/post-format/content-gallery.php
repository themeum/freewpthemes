...
<?php else: ?>
    <article id="post-<?php the_ID(); ?>" <?php post_class('wpestate-post wpestate-single-post wpestate-index-post'); ?>>
<?php endif; ?>

    <div class="featured-wrap blog-details-img">
        <div class="entry-content-gallery">
            <?php if(function_exists('rwmb_meta')){ ?>
                <?php $slides = get_post_meta( $post->ID,'wpestate_gallery_images',false); ?>
                    <?php if(count($slides) > 0) { ?>
                        <div id="blog-gallery-slider<?php echo esc_attr($post->ID); ?>" class="carousel slide blog-gallery-slider">
                            <!-- Wrapper for slides -->
                            <div class="carousel-inner">
                                <?php $slide_no = 1; ?>
                                <?php foreach( $slides as $slide ) { ?>
                                <div class="carousel-item <?php if($slide_no == 1) echo 'active'; ?>">
                                    <?php //$images = wp_get_attachment_image_src( $slide, 'wpestate-large' ); ?>

                                    <?php 
                                        $col = get_theme_mod( 'blog_column', 12 );
                                        if ($col == 3) {     
                                            $images = wp_get_attachment_image_src( $slide, 'wpestate-blog-medium' );      
                                        }elseif ($col == 4) {
                                            $images = wp_get_attachment_image_src( $slide, 'wpestate-blog-medium' );   
                                        }else{  
                                            $images = wp_get_attachment_image_src( $slide, 'wpestate-large' );  
                                        }
                                    ?> 
                                    <img class="img-responsive" src="<?php echo esc_url($images[0]); ?>" alt="<?php  esc_html_e( 'image', 'wpestate' ); ?>">
                                </div>
                                <?php $slide_no++; ?>
                                <?php } ?>
                            </div>
                            <!-- Controls -->
                            <a class="carousel-control-prev" href="#blog-gallery-slider<?php echo esc_attr($post->ID); ?>" data-slide="prev"><i class="fa fa-angle-left"></i></a>
                            <a class="carousel-control-next" href="#blog-gallery-slider<?php echo esc_attr($post->ID); ?>" data-slide="next"><i class="fa fa-angle-right"></i></a>
                        </div>
                <?php } ?>
            <?php } ?>
    ...
    <div class="wpestate-blog-content clearfix AA">
        <?php
            if (is_single()) { ?>
            ...
            <ul class="blog-post-meta clearfix"> 
                ...
                <?php if ( get_theme_mod( 'blog_category_single', false ) ): ?>
                    <li class="meta-category">
                        <i class="fa fa-folder"></i><?php printf(esc_html__('%s', 'wpestate'), get_the_category_list(', ')); ?>
                    </li>
                <?php endif; ?>
        ...
        <?php
            ...
            <ul class="blog-post-meta clearfix"> 
                ...
                <?php if ( get_theme_mod( 'blog_author', false ) ): ?>
                    <li class="meta-author">
                        <i class="fa fa-user"></i> <?php esc_html_e('By', 'wpestate') ?> <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>"><?php echo get_the_author_meta('display_name'); ?></a>
                    </li>
                <?php endif; ?>
