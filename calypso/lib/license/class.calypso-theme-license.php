...
class Calypso_Theme_License{
    ...
    public function license_page_asset_enquee(){
        wp_enqueue_style('plugin-license-handler', get_template_directory_uri().'/lib/license/license.css');
    }

	public function add_license_page(){
		add_submenu_page('calypso-options','License', 'License', 'manage_options', 'calypso-theme-license', array($this, 'license_form'));
	}
	...
	public function check_license_key(){
		if ( ! empty($_POST['thm_check_license_code'])){
			...
			$key  = sanitize_text_field($_POST['calypso_blog_license_key']);
				...
				<div class="themeum-box">
					...
					<form action="" method="post">
						...
						<p style="width: 100%; display: flex; flex-wrap: nowrap; box-sizing: border-box;">
							<input id="calypso_blog_license_key" name="calypso_blog_license_key" size="15" value="" class="regular-text code" style="flex-grow: 1; margin-right: 1rem;" type="text" placeholder="Enter your license key here" />
...
function support_admin_menu() {
    global $submenu;
    $url = 'https://www.themeum.com/docs/calypso-introduction/';
    $submenu['index.php'][] = array('Calypso Docs', 'manage_options', $url);
    $url = 'https://www.themeum.com/forums/forum/calypso/';
    $submenu['index.php'][] = array('Calypso Support Forum', 'manage_options', $url);

}

Calypso_Theme_License::init();