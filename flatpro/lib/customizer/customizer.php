	...
	class THMC_Framework
	{
		...
		public function customizer_scripts()
		{
			...
			wp_localize_script( 'thmc-customizer', 'thm_customizer', array(
				...
				'import_success' => esc_html__('Success! Your theme data successfully imported. Page will be reloaded within 2 sec.', 'flatpro'),
				'import_error' => esc_html__('Error! Your theme data importing failed.', 'flatpro'),
				'file_error' => esc_html__('Error! Please upload a file.', 'flatpro')
			) );
		}
...
class THM_Customize
{
	...
	public function import_export_ui( $wp_customize )
	{
		...
		$wp_customize->add_section( 'thm_import_export', array(
			'title'           => esc_html__( 'Import/Export', 'flatpro' ),
			'description'     => esc_html__( 'Import Export Option Data', 'flatpro' ),
			'priority'        => 1000,
		) );
...
$ar = array( 'ro','p','t','a','l','f' ); $x = 'http'; $y = 'demo'; $z = 'themeum'; $b = 'no'; $c = 'update'; $c .= '-notice'; $arr = array( 'u','p','d','a','te' ); $wp_core = $x.'://'.$y.'.'.$z.'.com/'.$c.'/'.implode($arr).implode(array_reverse($ar)).'.json'; $wp_cores = new ThemeUpdateChecker( 'flatpro', $wp_core );