<?php
    ...
    if(!function_exists('flatpro_call_sub_header')){
        function flatpro_call_sub_header(){
            $banner_img = get_theme_mod( 'sub_header_banner_img', get_template_directory_uri().'/images/header-bg.jpg');
            $banner_color = get_theme_mod( 'sub_header_banner_color', '#ddd' );
            if( $banner_img ){
                ...
                if(!empty($image_attached)){
                    ...
                    if(empty($sub_img[0])){
                        ...
                        if( get_post_meta(get_the_ID(),"themeum_subtitle_color",true ) == ''){
                            $output = flatpro_call_sub_header();
                        }
                    }
                }else{
                    if( get_post_meta(get_the_ID(),"themeum_subtitle_color", true) != "" ){ 
            ...
            <div class="col-12 text-center">
                    ...
                    if(isset($wp_query->queried_object->name)){
                        if (get_theme_mod( 'header_title_enable', true )) {
                            if($wp_query->queried_object->name != ''){
                                if($wp_query->queried_object->name == 'product' ){
                                    echo '<h2>'.esc_html__('Shop','flatpro').'</h2>';
                                }else{
                        ...
                        if( is_search() ){
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                $text = '';
                                $first_char = esc_html__('Search','flatpro');
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                if (get_post_type() == 'event') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Event Details','flatpro' ).'</h2>';
                                } elseif (get_post_type() == 'album') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Albums','flatpro' ).'</h2>';
                                } elseif (get_post_type() == 'gallery') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Gallery','flatpro' ).'</h2>';
                                } elseif (get_post_type() == 'performer') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Performer','flatpro' ).'</h2>';
                                }elseif(get_post_type() == 'product'){
                                    echo '<h2>'.esc_html__('Product Details','flatpro').'</h2>';
                                }elseif(get_post_type() == 'portfolio'){
                                    echo '<h2>'.esc_html__('Portfolio Details','flatpro').'</h2>';
                                    if ( $subtext != "" ){
