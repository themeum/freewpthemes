(function ( $ ) {
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$flatpro = this.$container.find( '.rwmb-map-flatpro' );
			this.flatpro = this.$flatpro[0];
		...
		initMapElements: function () {
			var defaultLoc = this.$flatpro.data( 'default-loc' ),
				latLng;
			...
			this.map = new google.maps.Map( this.flatpro, {
				center: latLng,
				zoom: 14,
				streetViewControl: 0,
			...
			$address.autocomplete( {
				source: function ( request, response ) {
					var options = {
						...
						'region': that.$flatpro.data( 'region' )
					};
