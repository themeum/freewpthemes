...
if ( $max_value && $min_value === $max_value ) {
	...
	<div class="product-quantity">
		<p class="title-quantity"><?php esc_html_e( 'quantity', 'winkel' ); ?></p>
		<div class="inner">
	        <a href="#" class="minus">
	            <i class="winkel winkel-substract-1"></i>
	        </a>
			<input type="number" class="input-text qty text" step="<?php echo esc_attr( $step ); ?>" min="<?php echo esc_attr( $min_value ); ?>" max="<?php echo esc_attr( 0 < $max_value ? $max_value : '' ); ?>" name="<?php echo esc_attr( $input_name ); ?>" value="<?php echo esc_attr( $input_value ); ?>" title="<?php echo esc_attr_x( 'Qty', 'Product quantity input tooltip', 'winkel' ) ?>" size="4" pattern="<?php echo esc_attr( $pattern ); ?>" inputmode="<?php echo esc_attr( $inputmode ); ?>" />
	        <a href="#" class="plus">
