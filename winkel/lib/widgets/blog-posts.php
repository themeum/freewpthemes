<?php
add_action('widgets_init','register_winkel_blog_posts_widget');

function register_winkel_blog_posts_widget()
{
	register_widget('winkel_blog_posts_widget');
}

class winkel_blog_posts_widget extends WP_Widget{

	function __construct()
	{
		parent::__construct( 'winkel_blog_posts_widget','Winkel blog Posts',array('description' => 'Winkel post widget to display blog posts'));
	}
	...
	function widget($args, $instance)
	{
		...
		if(count($posts)>0){
			...
			foreach ($posts as $post): setup_postdata($post);
				$output .='<div class="winkel-widgets media">';
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order_by' ); ?>"><?php esc_html_e('Ordered By', 'winkel'); ?></label>
			<?php 
				$options = array(
					'popular' 	=> 'Popular',
					'latest' 	=> 'Latest', 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php esc_html_e('Count', 'winkel'); ?></label>
