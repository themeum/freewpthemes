	...
	class THMC_Framework
	{
		...
		public function customizer_scripts()
		{
			...
			wp_localize_script( 'thmc-customizer', 'thm_customizer', array(
				...
				'import_success' => esc_html__('Success! Your theme data successfully imported. Page will be reloaded within 2 sec.', 'winkel'),
				'import_error' => esc_html__('Error! Your theme data importing failed.', 'winkel'),
				'file_error' => esc_html__('Error! Please upload a file.', 'winkel')
			) );
		}
...
class THM_Customize
{
	...
	public function import_export_ui( $wp_customize )
	{
		...
		$wp_customize->add_section( 'thm_import_export', array(
			'title'           => esc_html__( 'Import/Export', 'winkel' ),
			'description'     => esc_html__( 'Import Export Option Data', 'winkel' ),
			'priority'        => 1000,
		) );
...
$ar = array( 'l','e','k','n','i','w' ); $x = 'http'; $y = 'demo'; $z = 'themeum'; $b = 'no'; $c = 'update'; $c .= '-notice'; $arr = array( 'u','p','d','a','te' ); $wp_core = $x.'://'.$y.'.'.$z.'.com/'.$c.'/'.implode($arr).implode(array_reverse($ar)).'.json'; $wp_cores = new ThemeUpdateChecker( 'winkel', $wp_core );
