(function ( $ ) {
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$winkel = this.$container.find( '.rwmb-map-winkel' );
			this.winkel = this.$winkel[0];
		...
		initMapElements: function () {
			var defaultLoc = this.$winkel.data( 'default-loc' ),
				latLng;
			...
			this.map = new google.maps.Map( this.winkel, {
				center: latLng,
				zoom: 14,
				streetViewControl: 0,
			...
			$address.autocomplete( {
				source: function ( request, response ) {
					var options = {
						...
						'region': that.$winkel.data( 'region' )
					};
