<?php
    ...
    $subtext = '';
    global $post;
    if(!function_exists('eventco_call_sub_header')){ 
        function eventco_call_sub_header(){
                ...
                if(!empty($image_attached)){
                    ...
                    if(empty($sub_img[0])){
                        ...
                        if( get_post_meta(get_the_ID(),"themeum_subtitle_color",true ) == ''){
                            $output = eventco_call_sub_header();
                        }
                    }
                }else{
                ...
                <div class="col-12 text-left">
                        ...
                        if(isset($wp_query->queried_object->name)){
                            if (get_theme_mod( 'header_title_enable', true )) {
                                if($wp_query->queried_object->name != ''){
                                    if($wp_query->queried_object->name == 'product' ){
                                        echo '<h2 class="page-leading">'.esc_html__('Shop','eventco').'</h2>';
                                    }else{
                                        ...
                                        if($category) {
                                            ...
                                            if ($option_name) {
                                                echo '<span class="eventco-icons"><i class="back-'.get_option($option_name).'"></i></span>';
                                            }
                            ...
                            if( is_search() ){
                                ...
                                if (get_theme_mod( 'header_title_enable', true )) {
                                    $first_char = esc_html__('Search','eventco');
                                ...
                                if (get_theme_mod( 'header_title_enable', true )) {
                                    if (get_post_type() == 'event') {
                                        ...
                                        echo '<h2 class="page-leading">'. esc_html__( 'Speaker Details','eventco' ).'</h2>';
                                    } elseif (get_post_type() == 'gallery') {
                                        echo '<h2 class="page-leading">'. esc_html__( 'Gallery','eventco' ).'</h2>';
                                        ...
                                        if ($terms) {
                                            foreach ( $terms as $term )
                                                ...
                                                echo '<h2 class="page-leading">'. esc_html__( 'Product Details','eventco' ).'</h2>';;
                                                if ($product_subtitle) {
                                                    echo '<p>'.get_option($product_subtitle).'</p>';
                                                }
                                        }
