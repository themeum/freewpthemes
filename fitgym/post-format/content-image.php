...
<article id="post-<?php the_ID(); ?>" <?php post_class('fitgym-post fitgym-single-post'); ?>>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('fitgym-post fitgym-index-post'); ?>>
        ...
        <div class="<?php echo $post_img_class;?>">
            <?php
            if(has_post_thumbnail()) {
                if(is_single()){
                    ?>
                    <div class="img">
                        <?php the_post_thumbnail('fitgym-large', array('class' => 'img-fluid')); ?>
                    </div>
                    <?php
                }else{
                    ...
                    <a href="<?php the_permalink(); ?>">
                        <?php
                        if($col == 3 || $col == 4){
                            the_post_thumbnail('fitgym-blog-medium', array('class' => 'img-fluid'));
                        }
                        else{
                            the_post_thumbnail('fitgym-large', array('class' => 'img-fluid'));
                        }
                        ?>
                    </a>
                    <?php
            ...
            ?>
            <?php if(has_post_thumbnail() && !is_single() && $blog_date)  { ?>
                <div class="post-meta-1">
                    <h2><?php echo get_the_date('j', get_the_ID()); ?></h2>
                    <span><?php echo get_the_date('M, Y', get_the_ID()); ?></span>
                </div>
            <?php } ?>
        </div>
